...
# Récupération du lien vers la usbcam
usbcam_proc = subprocess.Popen(["./usbcam.sh"], stdout=subprocess.PIPE)
usbcam = usbcam_proc.stdout.read().decode("utf-8").strip().split()
if len(usbcam) > 0:
    print("usbcam : " + " ".join(usbcam))
...
def picture(sid, data):
    ...
    params = ["picture", usbcam_capture, 666, data]
...
if len(usbcam) > 0:
    ...
    print(usbcam_capture.get(3))
    print(usbcam_capture.get(4))

    # 1: Nom de la caméra
    # 2: VideoCapture de la caméra
    # 3: Liste des clients connectés au flux
    # 4: Référence vers le thread de streaming
    params = ["image", usbcam_capture, "usbcam", False]
    usbcam_thread = setInterval(1/float(fps), sendImage, params)
    print("usbcam thread created")

else:
    usbcam_capture = cv2.VideoCapture(0)
